...
const Icon = ({ media, list }: { media: string; list: Map<string, MediaIconProps> }) => (
  <div className="absolute left-1.5 flex flex-col items-center justify-center h-full">
    <img className="h-7 icon-shadow" src={list.get(media)?.icon} alt={`${media} icon`} loading="lazy" />
  </div>
);
        ...
        <div className="relative mt-1">
          ...
          >
            {filtered.length === 0 && query !== "" ? (
              <div className="relative cursor-default select-none py-2 px-4 text-gray-200">Nothing found.</div>
            ) : (
              filtered.map(media => (
                <Combobox.Option
                  key={media}
                ...
                >
                  {() => (
                    <React.Fragment>
                      <span className="block truncate mt-0.5">{list.get(media)?.label}</span>
